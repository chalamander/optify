...
if(!s7viewers.FlyoutViewer) {
	...
	s7viewers.FlyoutViewer.getCodeBase = function() {
		...
		for(var i=0; i<scriptTags.length;i++){
			...
			var result = /^\s*(http[s]?:\/\/[^\/]*)?(.*)(\/js\/FlyoutViewer\.js)/.exec(src);
	...
	s7viewers.FlyoutViewer.prototype.includeViewer = function () {
		...
		this.s7params.setViewer("504,5.2.3" + viewerName);
...
//
				...
				self.swatchesHeight = self.s7swatches.getHeight();
...
//
						if(self.viewerMode == "fixed") {
		                    ...
		                    self.container.setModifier({ "aspect": w /( w/assetRatio + self.swatchesHeight) });
						} else {
	...
	s7viewers.FlyoutViewer.prototype.initSiteCatalyst = function(params,inCallback) {
			...
			if (siteCatalystAsset.indexOf('/') != -1) {
				...
				if (isConfig2Exist){
                    ...
                    var jsp_src = this.getContentUrl() + '../AppMeasurementBridge.jsp?company=' + company;
